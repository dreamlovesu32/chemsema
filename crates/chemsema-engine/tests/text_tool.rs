...
fn first_glyph_anchor(label: &chemsema_engine::NodeLabel) -> Point {
    glyph_anchor(label, 0)
}

fn last_glyph_anchor(label: &chemsema_engine::NodeLabel) -> Point {
    glyph_anchor(label, label.glyph_polygons.len().saturating_sub(1))
}

fn normal_source_run(text: &str) -> chemsema_engine::LabelRun {
    chemsema_engine::LabelRun {
        text: text.to_string(),
        font_family: Some("Arial".to_string()),
        font_size: Some(chemsema_engine::DEFAULT_TEXT_FONT_SIZE_PT),
...
fn glyph_anchor(label: &chemsema_engine::NodeLabel, index: usize) -> Point {
    let polygon = label
        .glyph_polygons
        .get(index)
    ...
    let anchor_y = label.position.map(|position| {
        position[1]
            - label
                .font_size
                .unwrap_or(chemsema_engine::DEFAULT_MOLECULE_LABEL_FONT_SIZE_PT)
                * 0.39
    });
    Point::new(
    ...
    let document = json!({
        ...
        "styles": {
            "style_molecule_default": {
                "kind": "molecule",
                "stroke": "#000000",
                "strokeWidth": chemsema_engine::DEFAULT_BOND_STROKE,
                "fontFamily": "Arial",
        ...
        "resources": {
            "mol_001": {
                ...
                "data": {
                    ...
                    "nodes": [
                        {
                            ...
                            "label": {
                                ...
                                "runs": [{
                                    "text": "O",
                                    "fontFamily": "Arial",
                                    "fontSize": chemsema_engine::DEFAULT_TEXT_FONT_SIZE_PT,
                                    "fill": "#000000",
                                    "fontWeight": 400,
                                    "fontStyle": "normal",
                                    "underline": false,
                    ...
                    "bonds": [
                        { "id": "b1", "begin": "n1", "end": "n2", "order": 1, "strokeWidth": chemsema_engine::DEFAULT_BOND_STROKE, "lineStyles": {}, "lineWeights": {} },
                        { "id": "b2", "begin": "n1", "end": "n3", "order": 1, "strokeWidth": chemsema_engine::DEFAULT_BOND_STROKE, "lineStyles": {}, "lineWeights": {} },
                        { "id": "b3", "begin": "n1", "end": "n4", "order": 1, "strokeWidth": chemsema_engine::DEFAULT_BOND_STROKE, "lineStyles": {}, "lineWeights": {} }
                    ]
                }
            }
        }
...
fn assert_endpoint_target_near(session: &chemsema_engine::TextEditSession, expected: Point) {
...
fn begin_and_apply_text_object_edit_creates_text_scene_object() {
    ...
    let changed = engine.apply_text_edit(chemsema_engine::TextEditSession {
...
fn switching_to_select_selects_latest_text_object_or_edited_molecule_label() {
    ...
    let session = text_object_engine
        ...
        .expect("text session should be created");
    assert!(
        text_object_engine.apply_text_edit(chemsema_engine::TextEditSession {
            text: "reaction note".to_string(),
            ..session
        })
    );
    ...
    assert!(
        label_engine.apply_text_edit(chemsema_engine::TextEditSession {
            text: "NH3".to_string(),
            ..session
        })
    );
    ...
    assert!(
        abbreviation_engine.apply_text_edit(chemsema_engine::TextEditSession {
            text: "Ph".to_string(),
            ..session
        })
    );
...
fn preview_text_edit_layout_returns_kernel_caret_and_selection_geometry() {
    ...
    let layout = engine.preview_text_edit_layout(&TextEditLayoutRequest {
        session: chemsema_engine::TextEditSession {
...
fn centered_text_object_edit_layout_anchors_editor_to_box_center() {
    ...
    let session = chemsema_engine::TextEditSession {
...
fn text_object_default_box_height_wraps_descenders() {
    ...
    assert_eq!(
        session.line_height,
        Some(chemsema_engine::DEFAULT_TEXT_LINE_HEIGHT_PT)
    );
...
fn reopening_text_object_preserves_default_font_size_precision() {
    ...
    assert!((stored_font_size - chemsema_engine::DEFAULT_TEXT_FONT_SIZE_PT).abs() < 1.0e-6);

    let reopened = engine
        .begin_text_edit(px_point(120.0, 88.0))
        .expect("text object session should reopen");
    let reopened_font_size = reopened.font_size.expect("reopened session font size");
    assert!((reopened_font_size - chemsema_engine::DEFAULT_TEXT_FONT_SIZE_PT).abs() < 1.0e-6);
}
...
fn delete_tool_click_on_text_box_removes_text_object() {
    ...
    click(
        &mut engine,
        chemsema_engine::pt_to_px(translate[0] + bounds[2] * 0.5),
        chemsema_engine::pt_to_px(translate[1] + bounds[3] * 0.5),
    );
    assert!(engine
        .state()
        .document
...
fn preview_text_edit_layout_matches_committed_endpoint_label_geometry() {
    ...
    let session = engine
        ...
        .expect("endpoint session should be created");
    let edited_session = chemsema_engine::TextEditSession {
...
fn preview_text_runs_expands_chemical_source_runs_in_kernel() {
    ...
    let session = chemsema_engine::TextEditSession {
        ...
        text: "Fe2+".to_string(),
        source_runs: vec![chemsema_engine::LabelRun {
            text: "Fe2+".to_string(),
            font_family: Some("Arial".to_string()),
            font_size: Some(12.0),
            fill: Some("#000000".to_string()),
...
fn endpoint_label_stores_composite_abbreviation_recognition_metadata() {
    ...
    assert_eq!(expansion["schema"], "chemsema.functionalGroupExpansion.v1");
