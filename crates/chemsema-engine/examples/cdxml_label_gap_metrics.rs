...
use chemsema_engine::{
    parse_cdxml_document, render_document, ChemSemaDocument, MoleculeFragment, Point,
...
fn label_box_exit_distance(node_position: Point, unit: Point, bbox: [f64; 4]) -> Option<f64> {
    ...
    if dx.abs() > chemsema_engine::EPSILON {
        for x in [bbox[0], bbox[2]] {
            let t = (x - node_position.x) / dx;
            let y = node_position.y + dy * t;
            if t >= -chemsema_engine::EPSILON && y >= bbox[1] && y <= bbox[3] {
                candidates.push(t.max(0.0));
            }
        }
    }
    if dy.abs() > chemsema_engine::EPSILON {
        for y in [bbox[1], bbox[3]] {
            let t = (y - node_position.y) / dy;
            let x = node_position.x + dx * t;
            if t >= -chemsema_engine::EPSILON && x >= bbox[0] && x <= bbox[2] {
                candidates.push(t.max(0.0));
            }
        }
    }
...
fn segment_intersection_fraction(
    ...
    if denom.abs() <= chemsema_engine::EPSILON {
        return None;
    }
    ...
    for object in &document.objects {
        ...
        for bond in &fragment.bonds {
            ...
            for (node, other, endpoint_is_begin) in [(begin, end, true), (end, begin, false)] {
                ...
                if length <= chemsema_engine::EPSILON {
                    continue;
                }
                ...
                let visible = primitives
                    ...
                    .filter(|value| *value >= -chemsema_engine::EPSILON)
                    .min_by(|left, right| left.total_cmp(right));
