...
fn next_available_molecule_index(document: &ChemcoreDocument) -> usize {
...
mod tests {
    ...
    #[test]
    fn imported_center_aligned_label_anchor_uses_connection_side_glyph() {
        let document = parse_document_json(
            &json!({
                "format": { "name": "chemcore", "version": "0.1" },
                "document": {
                    "id": "doc_center_anchor_side",
                    "title": "center anchor side",
                    "page": { "width": 50.0, "height": 30.0, "background": "#ffffff" }
                },
                "objects": [{
                    "id": "obj_molecule_001",
                    "type": "molecule",
                        ...
                        "data": {
                            ...
                            "nodes": [{
                                ...
                                "label": {
                                    ...
                                    "fontFamily": "Arial",
                                    "fontSize": 10.0,
                                    "meta": { "import": { "cdxml": { "boundingBox": [3.61, 10.43, 16.39, 20.93] } } }
                                }
                            }, {
                                "id": "right_neighbor",
                                "element": "C",
        ...
        .expect("document should parse");
        let fragments = document.editable_fragments();
        let center_label_node = fragments
            .iter()
            .find_map(|entry| {
                entry
                    .fragment
                    .nodes
                    .iter()
                    .find(|node| node.id == "center_label")
            })
            .expect("center label node");
        let center_label = center_label_node.label.as_ref().expect("center label");

        assert!(
            glyph_center(center_label, 1).distance(center_label_node.point()) < 0.01,
            "center-aligned right-side bond should still anchor Ph on h: node={center_label_node:?}, label={center_label:?}"
        );
    }
