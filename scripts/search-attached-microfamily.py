...
def load_rows(phase_json: Path, summary_json: Path) -> list[dict]:
    phase_rows = json.loads(phase_json.read_text(encoding="utf-8"))["rows"]
    phase_by = {row["nodeId"]: row for row in phase_rows}
    summary_rows = json.loads(summary_json.read_text(encoding="utf-8"))
    rows: list[dict] = []
    for row in summary_rows:
        merged = dict(phase_by[row["nodeId"]])
        merged.update(row)
        rows.append(merged)
    return rows
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Search simple geometric predicates for attached-label replay micro-families."
    )
    ...
    rows = load_rows(phase_json, summary_json)
