...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run same-shell attached-label x atlas on top of a baseline with optional font-scale family.")
    ...
    if not baseline_ready:
        with tempfile.TemporaryDirectory(prefix="attached_x_baseline_", dir=str(out_dir)) as td:
            td_path = Path(td)
    ...
    for node_id in nodes:
        ...
        variant_env["CHEMCORE_EMF_ATTACHED_LABEL_REPLAY_NUDGE_EXPERIMENT_3"] = str(args.x_nudge)
        variant_env["CHEMCORE_EMF_ATTACHED_LABEL_REPLAY_NUDGE_NODE_FILTER_EXPERIMENT_3"] = node_id
        ...
        summary.append(
            {
                ...
                "labelIou": target["iou"],
                "labelDelta": float(target["iou"]) - float(base["iou"]),
                "text": target.get("text", ""),
                "fill": target.get("fill"),
            }
        )

    summary.sort(key=lambda row: (-float(row["globalDelta"]), -float(row["labelDelta"]), row["nodeId"]))
