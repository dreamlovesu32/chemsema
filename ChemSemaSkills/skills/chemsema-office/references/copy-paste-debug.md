...
chemsema-cli capture input.cdxml --target all --out all.png --scale 4 --pretty
```
...
Inspect the payload for document JSON, CDXML, SVG, custom ChemSema data, preview
...
```powershell
chemsema-cli detail input.cdxml --target object:obj_id --include-resource --out detail.json --pretty
```
...
```powershell
chemsema-cli copy input.cdxml --target all --payload payload-live.json --pretty
```

Paste into Word or PowerPoint, then check whether it is editable as a ChemSema
