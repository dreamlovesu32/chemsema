...
Use `--text` when ChemSema receives source text:

```powershell
chemsema-cli label-query --text CF3 --connection-angle 0 --pretty
chemsema-cli label-query --text NH2 --connection-angle 180 --connection-count 1 --pretty
```
...
```powershell
chemsema-cli label-query --visible-text F3C --connection-angle 0 --pretty
chemsema-cli label-query --visible-text H2N --connection-angle 180 --pretty
```

ChemSema proposes source labels that would render back to the visible text. If
...
that ChemSema's chemical label engine would otherwise rewrite. This is not a
...
Then attach labels through ChemSema label commands.
