...
```powershell
chemsema-cli inspect input.cdxml --include summary,objects,molecules,resources,styles --out inspect.json --pretty
```
...
```powershell
chemsema-cli context input.cdxml --target molecule:1 --expand-rel 0.2 --out context.json --capture-out context.png --scale 6 --pretty
```
...
```powershell
chemsema-cli detail input.cdxml --target molecule:0 --include-resource --out detail.json --pretty
chemsema-cli detail input.cdxml --target node:n_1 --out node.json --pretty
chemsema-cli detail input.cdxml --target bond:b_1 --out bond.json --pretty
```
...
```powershell
chemsema-cli bundle input.cdxml --target molecule:0 --out-dir molecule-0-bundle --context-radius 40 --capture-format png --subset-format ccjs --pretty
```
