...
```powershell
chemsema-cli new commands.json --out output.ccjs --results results.json --document-json output-document.json --pretty
chemsema-cli run input.cdxml commands.json --out edited.cdxml --results results.json --inspect-after summary,objects,molecules --pretty
```

The command input is a JSON object, array, or
`chemsema.command-transaction.v1` envelope. Use `-` to read JSON from stdin.
...
`add-bond` creates a ChemSema bond. For double bonds, omit placement unless the
...
Drawing agents should use these planning queries to avoid duplicating ChemSema
GUI placement logic.
