#!/usr/bin/env python3
"""Locate a ChemSema CLI runtime for bundled skill helpers."""
...
class ChemSemaRuntimeNotFound(FileNotFoundError):
    """Raised when a helper cannot find chemsema-cli."""
...
def executable_name() -> str:
    return "chemsema-cli.exe" if platform.system().lower() == "windows" else "chemsema-cli"


def executable_from_env() -> Path | None:
    value = os.environ.get("CHEMSEMA_CLI")
    if not value:
        return None
    path = Path(value)
...
def executable_from_path() -> Path | None:
    found = shutil.which("chemsema-cli")
...
def find_cli() -> tuple[list[str], Path | None, str]:
    ...
    if env_exe:
        return [str(env_exe)], None, "CHEMSEMA_CLI"
    ...
    tag = platform_tag()
    raise ChemSemaRuntimeNotFound(
        "chemsema-cli was not found. Install the self-contained ChemSema CLI skill "
        f"with assets/bin/{tag}/{executable_name()}, install ChemSema CLI on PATH, "
        "or set CHEMSEMA_CLI to an executable path. Source checkout builds are "
        "handled by the chemsema-development skill."
    )
