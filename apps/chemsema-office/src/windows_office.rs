...
impl OleObjectPayload {
    ...
    fn from_clipboard(payload: ClipboardPayload) -> Self {
        ...
        let document_json = payload
            ...
            .unwrap_or(fallback.chemsema_document_json);
...
fn ole_edit_session_path_for_object(object: *mut ChemSemaOleObject) -> Option<PathBuf> {
    if object.is_null() {
        return None;
    }
...
unsafe fn notify_ole_object_changed(object: *mut ChemSemaOleObject) {
    if object.is_null() {
        return;
    }
...
unsafe fn replace_object_storage(object: *mut ChemSemaOleObject, storage: *mut c_void) {
...
fn chemsema_object_query_interface(
    object: *mut ChemSemaOleObject,
    riid: *const GUID,
    out: *mut *mut c_void,
) -> i32 {
...
fn chemsema_object_add_ref(object: *mut ChemSemaOleObject) -> u32 {
    if object.is_null() {
        return 0;
    }
...
fn chemsema_object_release(object: *mut ChemSemaOleObject) -> u32 {
    if object.is_null() {
        return 0;
    }
...
unsafe fn owner_from_part<T>(this: *mut c_void) -> *mut ChemSemaOleObject {
    if this.is_null() {
        return null_mut();
    }
...
fn chemsema_document_stream_payload(payload: &OleObjectPayload) -> Vec<u8> {
...
unsafe fn launch_desktop_for_object(object: *mut ChemSemaOleObject) -> Result<(), String> {
    if object.is_null() {
