...
fn preview_text_lines(text: &str, runs: &[chemsema_engine::LabelRun]) -> Vec<Vec<PreviewTextRun>> {
    if runs.is_empty() {
        return text
            .lines()
            .map(|line| {
...
fn preview_label_context_from_document(document: &ChemSemaDocument) -> PreviewLabelContext {
...
fn preview_bond_context_from_document(document: &ChemSemaDocument) -> PreviewBondContext {
    let mut infos = BTreeMap::new();
    for object in document
        .scene_objects()
        .into_iter()
...
fn preview_label_is_simple_single_run(label: &chemsema_engine::NodeLabel) -> bool {
