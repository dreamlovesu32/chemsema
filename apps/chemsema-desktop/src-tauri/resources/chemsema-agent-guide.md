# ChemSema Agent Guide

This guide gives automation agents a source-code-independent map of ChemSema.
...
installing, then call `chemsema-cli` directly.
...
Use `chemsema-cli --version` for a one-line shell version check. Use
`chemsema-cli version --pretty` and `chemsema-cli schema protocol --pretty` when
an agent needs product and protocol versions as JSON.

This quick guide is installed as `chemsema-agent-guide.md`. The detailed English
CLI guide is installed as `chemsema-cli-guide.md`. To include guide Markdown in
JSON, use:

```powershell
chemsema-cli guide --kind agent --include-content --out chemsema-agent-guide.json --pretty
chemsema-cli guide --kind detailed --include-content --out chemsema-cli-guide.json --pretty
```
...
```powershell
chemsema-cli context input.cdxml --target molecule:1 --expand-left 40 --expand-right 120 --expand-rel-y 0.25 --out context.json --capture-out context.png --scale 6 --pretty
```
...
chemsema-cli detail input.cdxml --target molecule:0 --out molecule-detail.json --pretty
chemsema-cli detail input.cdxml --target node:n_1 --out node-detail.json --pretty
chemsema-cli detail input.cdxml --target bond:b_1 --out bond-detail.json --pretty
```
...
```powershell
chemsema-cli capture input.cdxml --target molecule:0 --out molecule.png --scale 6 --expand-rel 0.15 --pretty
```

Multiple targets use the same bounds logic as the GUI selection box:

```powershell
chemsema-cli capture input.cdxml --target object:obj_a --target object:obj_b --out selection.png --width 1800 --pretty
```

If `--out` is omitted, `capture` writes a PNG to the OS temp `chemsema-cli`
...
```powershell
chemsema-cli capture input.cdxml --target object:obj_shape_001 --out object.png --width 1800 --expand 12 --pretty
```
...
or nested `double: {"placement": "left"}`. Omit it to keep ChemSema's automatic
double-bond side placement.

Run commands against an existing document:

```powershell
chemsema-cli run input.cdxml commands.json --out edited.cdxml --results run-results.json --pretty
```
...
```powershell
chemsema-cli copy input.cdxml --target molecule:0 --pretty
chemsema-cli copy input.cdxml --target object:obj_shape_001 --payload payload.json --no-copy --pretty
```
...
```powershell
chemsema-cli captur input.cdxml --target molecule:0 --out crop.png
```
...
```powershell
chemsema-cli capture input.cdxml --target molecule:0 --out crop
```
