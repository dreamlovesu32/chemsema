...
```powershell
chemcore-cli new commands.json --out molecule.ccjs --results results.json --document-json molecule-doc.json --pretty
```

6. Inspect:

```powershell
chemcore-cli targets molecule.ccjs --out targets.json --pretty
chemcore-cli capture molecule.ccjs --target all --out molecule.png --scale 8 --pretty
```
...
```powershell
chemcore-cli run input.ccjs commands.json --out edited.ccjs --results results.json --pretty
```
