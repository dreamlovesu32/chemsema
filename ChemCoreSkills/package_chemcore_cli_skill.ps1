param(
    [string]$OutDir = "dist\chemcore-skills",
    [switch]$Clean
)
...
$skillSource = Join-Path $root "skills\chemcore-cli"
...
$packageSkillDir = Join-Path $stagingRoot "chemcore-cli"
...
$zipName = "chemcore-cli-skill-$version-$platforms-unsigned.zip"
...
@"
ChemCore CLI Skill Runtime Notice

This package includes an unsigned prebuilt ChemCore CLI runtime. Verify the
package with SHA256SUMS.txt before installing it. The runtime assets are also
listed in chemcore-cli/assets/runtime-manifest.json with size and SHA256 values.

If you do not want to run the bundled runtime, install a trusted ChemCore CLI
separately and set CHEMCORE_CLI to its executable path.
