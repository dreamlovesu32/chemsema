# ChemSema v1.0.0-beta.4

这个 beta 是一次覆盖面较大的交互与自动化版本。它改进了大文件编辑、CDXML 渲染/导入保真、hover 和拖拽反馈、浏览器文件处理、回归覆盖，并新增 headless CLI，让 agent 和脚本可以在不驱动 GUI 的情况下调用 ChemSema。

## 主要变化

- 新增 `chemsema-cli` 和直接 engine 命令，支持 headless 文档检查、转换、导出、编辑，以及结构化 JSON 执行报告。
- 新增 `--document-json`、`--inspect-after`，并改进 `.json`/`.ccjs` 处理，让脚本和 agent 更容易交换 ChemSema 文档。
