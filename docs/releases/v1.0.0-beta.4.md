...
- Added `chemsema-cli` and direct engine commands for headless inspection, conversion, export, document editing, and structured JSON execution reports.
- Added `--document-json`, `--inspect-after`, and improved `.json`/`.ccjs` handling for scriptable ChemSema document exchange.
