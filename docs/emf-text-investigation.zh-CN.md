...
## 2026-05-18 Office ?? invalid marker ????

### ????
- `ppt-sample-wjg4/slide12.oleObject2` ?????????????? knockout?????? bbox?
- ????
  - `text-only` ???
  - `bond-only` ???
  - `graphic-only` ????????
...
### ??
- `chemcore-engine` ? `labelRecognition.status = invalid` ????????
  - ?? `Rect`?invalid label marker?
  - ?? `Circle`?invalid charge marker?
- Office ??????? `DocumentGraphic` ??????????? PPT ?????????????????????

### ????
- `apps/chemcore-office/src/windows_office/emf_preview/renderer.rs`
  - ?? `preview_is_invalid_marker_primitive(...)`
  - ??? `office_preview_primitive_visible(...)` ????? invalid marker
  - ????????
    - `CHEMCORE_EMF_SHOW_INVALID_MARKERS=1`
- ????
  - `wjg4/slide12` ???????? `178`
  - ??????? `0`
...
- ???????????
  - `attached-group-above + multiline + black`
  - `attached-group + multiline + black`
  ? baseline / line-step / stacking ???


## 2026-05-18 ?? multiline black attached-label family

### ??
- ???? node-specific ?????????????? multiline attached-label family ???????
- ??????
  - `fill = #000000`
  - `text_anchor = start`
  - `layout in {attached-group-above, attached-group}`
  - `line_count > 1`
- ???`y = -3 px`

### ????
- `preview_attached_label_replay_default_family_y_nudge_px(...)`
- ? `preview_attached_label_replay_y_nudge_px(...)` ????????? family nudge

### ????
- ?????
  - `preview_default_multiline_black_attached_labels_nudge_up`
- ?? invalid marker ????????????

### 19 ? PPT same-shell ??????
?????
- `tmp/ppt-generalization-default-multiblack`

???
- `count = 19`
- `avgBestIou = 0.3804941082`
- `avgDx = 5.0`
- `avgDy = 6.4736842105`
- `failures = 0`

???? generalized baseline????? family????
- `avgBestIou ? 0.37775`
...
### ????
- ????????? bug ????????????????????????? multiline black family?
- ????????
  - `attached-group-above + single-line + black` ???????
  - ? lateral black labels ? replay ??
...
- ?????`cch`?? overlay ????????
- ?????????
  - `b7645df`???? invalid marker?
  - `c00c15b`?? `multiline black attached-label -> y:-3px`
  - ??????????? structure-level replay ??

### ?????
- ???????
  - `tmp/ppt-sample-cch/same-shell-compare/slide2.oleObject1.payload.json`
- ?????????? Word `CopyAsPicture`
  - `b7645df`???????? invalid marker???????? multiline family ????
  - ??? HEAD???? invalid marker ?? + multiline family ????
- ????
  - `tmp/cch-ab/pre_multiblack.wordcopy.png`
  - `tmp/cch-ab/current.wordcopy.png`
- ? ChemDraw ????? `best-shift`
- ??????是否有像素变化

### ??
- `pre_multiblack -> ChemDraw`
  - `best_iou = 0.3684245134`
  - `dx = 4`
  - `dy = 6`
- `current -> ChemDraw`
  - `best_iou = 0.3684245134`
...
- ??? `cch` ????
  - ???? invalid marker ?????????????
  - `multiline black attached-label` ??? `y:-3px` ??????????
- ?????? Word ??????**像素完全一致**
- ??
  - 用户在 overlay 里看到的结构偏移，**不是最近标签框修复引入的回归**
  - ??????`cch`???????????? packaged replay / structure-level ??

### ??
- ?????? `multiline black attached-label -> y:-3px` family ??
  - ??? 19 ????????略有帮助
  - ???**不是**解决 `cch` ????“结构和文字一起偏”的主线
- ?????再把 `cch` ?????归因到最近的标签框/多行标签修补
